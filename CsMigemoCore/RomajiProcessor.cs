...
namespace CsMigemo
{
    static class RomajiProcessor
    {
        public class RomanEntry
        {
            ...
            public int Remain;
            public uint Index;
            public RomanEntry(string roman, string hiragana, int remain)
            {
                Roman = roman;
                Hiragana = hiragana;
                Remain = remain;
            ...
            public static uint CalculateIndex(string roman)
            {
                return CalculateIndex(roman, 0, 4);
            }
            public static uint CalculateIndex(string roman, int start, int end)
            {
                uint result = 0;
                for (var i = 0; i < 4; i++)
                {
                    var index = i + start;
                    var c = index < roman.Length && index < end ? roman[index] : (char)0;
                    result |= (byte)c;
                    if (i < 3)
                    {
                        result <<= 8;
        ...
        public class RomajiPredictiveResult
        {
            public readonly string EstaglishedHiragana;
            public readonly ISet<string> PredictiveSuffixes;
            public RomajiPredictiveResult(string estaglishedHiragana, ISet<string> predictiveSuffixes)
            {
                EstaglishedHiragana = estaglishedHiragana;
                PredictiveSuffixes = predictiveSuffixes;
            }
        }
        ...
        public static string ConvertRomajiToHiragana(string romaji)
        {
            ...
            var hiragana = new StringBuilder();
            var start = 0;
            var end = 1;
            while (start < romaji.Length)
            {
                var lastFound = -1;
                var lower = 0;
                var upper = ROMAN_INDEXES.Length;
                while (upper - lower > 1 && end <= romaji.Length)
                {
                    var lowerKey = RomanEntry.CalculateIndex(romaji, start, end);
                    lower = Array.BinarySearch(ROMAN_INDEXES, lower, upper - lower, lowerKey);
                    if (lower >= 0)
                    {
                        lastFound = lower;
                    }
                    ...
                    uint upperKey = lowerKey + (uint)(1 << (32 - 8 * (end - start)));
                    upper = Array.BinarySearch(ROMAN_INDEXES, lower, upper - lower, upperKey);
                    if (upper < 0)
                    {
                        upper = -upper - 1;
                    }
                ...
                if (lastFound >= 0)
                {
                    var entry = ROMAN_ENTRIES[lastFound];
        ...
        public static ISet<RomanEntry> FindRomanEntryPredicatively(string roman, int offset)
        {
            var lastFound = -1;
            var startIndex = 0;
            var endIndex = ROMAN_INDEXES.Length;
            for (var i = 0; i < 4; i++)
            {
                ...
                var startKey = RomanEntry.CalculateIndex(roman, offset, offset + i + 1);
                startIndex = Array.BinarySearch(ROMAN_INDEXES, startIndex, endIndex - startIndex, startKey);
                if (startIndex >= 0)
                {
                ...
                var endKey = startKey + (uint)(1 << (24 - 8 * i));
                endIndex = Array.BinarySearch(ROMAN_INDEXES, startIndex, endIndex - startIndex, endKey);
                if (endIndex < 0)
                {
                    endIndex = -endIndex - 1;
                }
                ...
                {
                    return new HashSet<RomanEntry>() { ROMAN_ENTRIES[startIndex] };
                }
            }
            var result = new HashSet<RomanEntry>();
        ...
        public static RomajiPredictiveResult RomajiToHiraganaPredictively(string romaji)
        {
            if (romaji.Length == 0)
            {
                return new RomajiPredictiveResult("", new HashSet<string>() { "" });
            }
            var hiragana = new StringBuilder();
            var start = 0;
            var end = 1;
            while (start < romaji.Length)
            {
                ...
                while (upper - lower > 1 && end <= romaji.Length)
                {
                    ...
                    var upperKey = lowerKey + (uint)(1 << (32 - 8 * (end - start)));
                    upper = Array.BinarySearch(ROMAN_INDEXES, lower, upper - lower, upperKey);
                    if (upper < 0)
                    {
                        upper = -upper - 1;
                ...
                if (end > romaji.Length && upper - lower > 1)
                {
                    ...
                    {
                        var re = ROMAN_ENTRIES[i];
                        if (re.Remain > 0)
                        {
                            var set2 = FindRomanEntryPredicatively(romaji, end - 1 - re.Remain);
                            foreach (var re2 in set2)
                            {
                                if (re2.Remain == 0)
                                {
            ...
            return new RomajiPredictiveResult(hiragana.ToString(), new HashSet<string>() { "" });
        }

        private readonly static RomanEntry[] UNSORTED_ROMAN_ENTRIES = new RomanEntry[] {
    new RomanEntry("-", "ー", 0),
    new RomanEntry("~", "〜", 0),
    new RomanEntry(".", "。", 0),
    new RomanEntry(",", "、", 0),
    ...
    new RomanEntry("who", "うぉ", 0) };
        ...
        private static readonly RomanEntry[] ROMAN_ENTRIES = new List<RomanEntry>(UNSORTED_ROMAN_ENTRIES).OrderBy(x => x.Index).ToArray();
        private static readonly uint[] ROMAN_INDEXES = new List<RomanEntry>(ROMAN_ENTRIES).Select(x => x.Index).ToArray();
    }
}
